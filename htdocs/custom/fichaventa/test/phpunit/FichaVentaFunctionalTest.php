...
/**
 * \file    test/phpunit/FichaVentaFunctionalTest.php
 * \ingroup fichaventa
...
/**
 * Class FichaVentaFunctionalTest
 *
 ...
 *
 * @package Testfichaventa
 */
class FichaVentaFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modFichaVenta')->click();
		} else {
			// Disable the module
			$this->byHref('modFichaVenta')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/fichaventa/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('fichaventa/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/fichaventa/admin/about.php');
		$this->authenticate();
		return $this->assertContains('fichaventa/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('fichaventawidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modFichaVenta_FichaVentaTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modFichaVenta_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
